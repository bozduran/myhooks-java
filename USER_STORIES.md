...
- **Done:** US-00 (scaffold), US-01 (`xmlspan`), US-02 (`edit`), US-03 (`textrules`), US-04 (`diffui`), US-05 (`discover`+`git`), US-06 (`step`), US-07 (`jrutil`), US-08 (`commitmsg`), US-09 (`format`), US-10 (`sort`), US-11 (`textcheck`), US-12 (`clear`), US-13 (`validate`).
...
- **Next:** US-14 — `refactor(report): port include-chain report with includegraph`.
